...
function cacheFunction(cb) {
    ...
    cacheFunction debe retornar una función. Esta función debe aceptar un argumento (arg) e invocar a cb con ese argumento; ç
    ...
    return function memory(n) {
        ...
        if (arr.length > 0) {
            for (let i = 0; i < arr.length; i++) {
                if (arr1[i] === n) return arr[i];
                if (arr1[i] !== n && i === arr.length - 1) {
                    temp = cb(n);
                    arr.unshift(temp);
                    arr1.unshift(n);
                    return temp;
